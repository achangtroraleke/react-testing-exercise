...
it("works when you click on the right arrow", function() {
  ...
  // expect the left arrow's display to be none
  ...
  // expect right arrow's display to be none when on last image.
  fireEvent.click(rightArrow);
...
it("works when you click on the left arrow", function(){
  ...
  // expect the first image show, but not the second
  expect(
    container.querySelector('img[alt="testing image 1"]')
  ).toBeInTheDocument();
  expect(
  ...
  // expect left arrow's display to be none when on first image.
